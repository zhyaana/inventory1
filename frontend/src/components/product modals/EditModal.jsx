...
const EditModal = ({ updatedProduct, onClose , editProduct ,categories}) => {

    const category = categories.filter(cate=>{
       return cate.categoryid === updatedProduct.categoryid;
    })
                        ...
                        <form className="space-y-4" action="#">
                            ...
                            <div className="mb-2">
                                <select name="categoryname" value={product.categoryname} className="bg-gray-100 text-gray-900 mt-2 p-3 rounded-lg outline outline-1 focus:outline-none  focus:shadow-outline block w-full p-2.5" onChange={handlerChange}>
                                    <option value={category[0].categoryname}>{category[0].categoryname}</option>
                            ...
                            <div className="mb-5">
                                <button type="button" className="mr-3 ml-10 w-[200px] text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800" onClick={()=>editProduct(product , product.productid)}>Edit</button>
                            </div>
